...
import * as auth from "./customAuth.js";
...
router.post("/login",async  (req, res, next)=>{
    try{
        ...
        const valid_pwd = auth.validatePassword(req.body.password, user_found.hash, user_found.salt);
        if(valid_pwd){
            const jwt_token = auth.issueJWT(user_found);
...
router.post("/register", async (req, res, next)=>{
    const saltHash = auth.genPasswordHash(req.body.password);
    ...
    try{
        ...
        const jwtToken = auth.issueJWT(newUser);
 ...
 */
router.get("/secret-page", auth.customAuthMiddleware, (req, res, next)=> {
